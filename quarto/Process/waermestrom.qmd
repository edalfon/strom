...
%sql INSTALL sqlite;
%sql LOAD sqlite;
%sql SELECT * FROM sqlite_scan('data/2022-12-28-ecas-export.db', 'meter');
```
...
CREATE OR REPLACE VIEW waermestrom_sqlite AS 
SELECT 
  meterid, 
...
FROM sqlite_scan('data/2022-12-28-ecas-export.db', 'reading') 
WHERE meterid = 2 OR meterid = 3
;
SELECT * FROM waermestrom_sqlite;
```

Ideally, there would be one measurement for each tarif, for every date (minute). But we cannot guarantee that's the case (e.g. measurement for one tarif can be at 13:08:59 and for the other at 12:09:00). So let's see if that's the case.
```{python}
%%sql 
...
CREATE OR REPLACE VIEW waermestrom_nulls AS
WITH
ws181 AS (
  SELECT 
    'Hoch' AS tariff,
...
CREATE OR REPLACE VIEW waermestrom_nonulls AS
SELECT
  date,
  value_hoch, value_niedrig, 
  -- calculate minutes diff with previous and next date, to see which is closer
  date_sub('minute', lag(date) over(order by date), date) AS minutes_lag,
  date_sub('minute', date, lead(date) over(order by date)) AS minutes_lead,
...
ORDER BY date
;
SELECT * FROM waermestrom_nonulls LIMIT 20;
```
...
CREATE OR REPLACE VIEW waermestrom AS
SELECT 
  date,
  value,
...
;
SELECT * FROM waermestrom LIMIT 20;
```
...
CREATE OR REPLACE VIEW waermestrom_minute AS
...
;
SELECT * FROM waermestrom_minute ORDER BY minute LIMIT 10;
```
...
CREATE OR REPLACE VIEW waermestrom_filledin AS
SELECT
  *,
  CASE 
    WHEN cm IS NULL THEN
      (SELECT cm 
      FROM waermestrom_minute t2 
      WHERE t2.minute > t1.minute and cm is not null 
      ORDER BY minute
      LIMIT 1)
    else cm 
  END AS cm
FROM waermestrom_minute t1
ORDER BY t1.minute
;
SELECT * FROM waermestrom_filledin ORDER BY minute LIMIT 100;
```
