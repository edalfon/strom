...
def get_latest_file_path(folder_path):
    ...
    # Sort the list of files in lexicographical order
...
```{python}
latest_file = get_latest_file_path('./data/')
```
...
Please note that you cannot use this as prepared statements to create views.
Therefore, below we will now use tables.
...
SELECT 
  ...
  value_hoch_fix - lag(value_hoch_fix) over(order by date) AS consumption_hoch,
  value_niedrig_fix - lag(value_niedrig_fix) over(order by date) AS consumption_niedrig,
...
SELECT
  ...
  LAST_VALUE(cm_hoch IGNORE NULLS) OVER(
    ORDER BY minute ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS cm_hoch,
  LAST_VALUE(cm_niedrig IGNORE NULLS) OVER(
    ORDER BY minute ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS cm_niedrig
