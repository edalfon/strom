...
#| include: false
%%sql
SELECT * FROM strom_intervals
WHERE meterid <> 1 LIMIT 100
;
```
...
SELECT 
  ...
  date_diff('minute', First, Last) AS Mins, 

  24.0 * 60.0 * SUM(cm * interval_minutes) / Mins AS "Use/Day", 
  24.0 * 60.0 * SUM(cm * interval_minutes) / Mins AS "check", 
